<script lang="ts">
	...
	const voices: string[] = ['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer'];
	const voiceModels: string[] = ['tts-1', 'tts-1-hd'];
	...
	async function listen() {
		...
		let found = audios.find(
			(audio) => audio.model === model && audio.voice === voice && audio.speed === data.audioSpeed
		);
		if (found) {
			return;
		}
...
<div class="flex flex-col justify-center">
	...
	<Accordion>
		<AccordionItem>
			<svelte:fragment slot="summary">OpenAI</svelte:fragment>
				...
				<div class="flex flex-col mt-5">
					...
					<label class="label">
						...
						<input
							class="input"
							type="text"
							placeholder="OpenAI vector store id"
							bind:value={data.vectorStoreId}
							disabled
						/>
					</label>
		...
		<AccordionItem>
			<svelte:fragment slot="summary">Text to speech (OpenAI)</svelte:fragment>
			<svelte:fragment slot="content">
				<label class="label">
				...
				<!-- listen -->
				<button class="btn variant-filled-primary" on:click={listen}> Listen </button>
	...
	<div class="flex flex-row mt-5">
		<button
			class="btn {saved ? 'variant-filled' : 'variant-filled-primary'}"
			disabled={saved === Status.SAVED}
			on:click={save}
		>
		...
		<button
			class="btn variant-filled-error ml-2"
			use:popup={popupHover}
			on:click={deleteAndReset}
			disabled={deleting || !createdOpenAI}
		>
			{#if deleting}
				<Fa icon={faSpinner} class="animate-spin" />
